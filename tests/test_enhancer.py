...
def test_creativity_parameter_passing():
...
def test_enhance_removes_quotes(enhancer):
    """Checks that the enhance method removes quotes from the generated caption."""
    mock_response = MagicMock()
    mock_response.text = '"A beautiful landscape."'
    
    with patch.object(enhancer.model, 'generate_content', return_value=mock_response):
        result = enhancer.enhance("landscape")
...
def test_enhance_edge_cases(enhancer, input_text, expected_output):
    """Checks that the enhance method returns the input unchanged for invalid inputs."""
    assert enhancer.enhance(input_text) == expected_output
...
def test_enhance_general_error_handling(enhancer):
    """Checks that EnhancementError is raised for general failures."""
    with patch.object(enhancer.model, 'generate_content', side_effect=Exception("Random Error")):
        with pytest.raises(EnhancementError):
            enhancer.enhance("a red car")

def test_network_error_handling(enhancer):
    """Checks that ModelNetworkError is raised when there is no internet connection."""
    with patch.object(enhancer.model, 'generate_content', side_effect=Exception("Failed to establish a network connection")):
        with pytest.raises(ModelNetworkError) as excinfo:
            enhancer.enhance("a dog")
        assert "internet connection" in str(excinfo.value).lower()

def test_api_key_error_handling(enhancer):
    """Checks that APIConfigurationError is raised for invalid API key."""
    with patch.object(enhancer.model, 'generate_content', side_effect=Exception("403 Forbidden: api_key is invalid")):
        with pytest.raises(APIConfigurationError) as excinfo:
            enhancer.enhance("a cat")
        assert "Invalid Gemini API Key" in str(excinfo.value)

def test_init_unexpected_api_failure():
...
def test_resource_limit_error_handling(enhancer) -> None:
    """Checks that ResourceLimitError is raised when API quota is exhausted."""
    with patch.object(enhancer.model, 'generate_content', side_effect=Exception("429 Too Many Requests: quota exceeded")):
        with pytest.raises(ResourceLimitError) as excinfo:
            enhancer.enhance("a sunset")
